"""Fusion finding generator.

Generates findings that are only detectable when both CMDB assessment
data and Pathfinder behavioral data are available.
"""
...
class FusionFindingGenerator:
    ...
    def generate(self, cmdb_cis: list[dict[str, object]]) -> list[FindingResponse]:
        ...
        Args:
            cmdb_cis: List of CI records from ServiceNow CMDB.

        Returns:
            List of fusion-only findings.
        """
    ...
    def _detect_ghost_cis(self, cmdb_cis: list[dict[str, object]]) -> list[FindingResponse]:
        ...
        for ci in cmdb_cis:
            ...
            # CI marked operational in CMDB but Pathfinder says deprecated/no-record
            if status == "1":  # Operational
                if record and record.traffic_state.value == "deprecated":
                    ...
                    # Not monitored — can't determine ghost status
                    pass
